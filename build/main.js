...
class BoschSmartHomeCamera extends utils.Adapter {
    ...
    _httpClient = (0, auth_1.createCloudHttpClient)();
     ...
     */
    _lastCloudReconcile = new Map();
    static CLOUD_RECONCILE_MIN_MS = 45000;
    /**
     ...
     */
    static FCM_SAFETY_POLL_MS = 300_000;
    /**
    ...
    _motionLightCache = new Map();
    _ambientLightCache = new Map();
    // v1.1.0: DP-name → /notifications API-key map for the per-type toggles.
    static NOTIFY_TYPE_MAP = {
    ...
    async _maybeAnnounceMaintenanceState(mw, state) {
        ...
        const bodyParts = [mw.title || "Maintenance notice"];
        if (when) {
            bodyParts.push(when);
        }
        if (state === "active") {
     ...
     */
    async _pruneOrphanedCameraObjects(liveIds) {
        // Refuse to run if the live set is empty — would wipe everything.
        if (liveIds.size === 0) {
            this.log.warn("_pruneOrphanedCameraObjects: live camera set is empty — skipping prune (safety guard)");
            return;
        }
        let pruned = 0;
        try {
                ...
                for (const key of Object.keys(allObjs)) {
                    ...
                    if (shortKey === prefix || shortKey.startsWith(`${prefix}.`)) {
                        try {
                            ...
                            this.log.debug(`_pruneOrphanedCameraObjects: could not delete ${shortKey}: ${delErr instanceof Error ? delErr.message : String(delErr)}`);
                        }
                    }
                }
                pruned++;
                this.log.info(`Pruned orphaned camera subtree cameras.${uuid} (${deletedInSubtree} object(s) removed — camera no longer in Bosch account)`);
            }
        }
        catch (err) {
            this.log.warn(`_pruneOrphanedCameraObjects failed: ${err instanceof Error ? err.message : String(err)}`);
        }
        if (pruned > 0) {
    ...
    async upsertSession(camId, session) {
        try {
            ...
            if (persistent) {
                ...
                this.log.debug(`TLS proxy for camera ${camId.slice(0, 8)}: inner proxy on ephemeral ` +
     ...
     */
    _maxSessionDurationParam(session) {
        const configured = Number(this.config.stream_max_session_duration);
        const override = Number.isFinite(configured) && configured >= 600 ? Math.min(configured, 21_600) : 0;
        if (override > 0) {
            return override;
        }
    ...
    async onReady() {
        ...
        this._fcmListener.on("heal", (info) => {
            this.log.info(`FCM self-heal: persisted credentials rejected (${info.detail}) — ` +
    ...
    async _startPersistentEndpoints(cameras) {
        ...
        for (const cam of cameras) {
            ...
            try {
                const handle = await (0, lazy_stream_1.startLazyFrontDoor)({
                    cameraId: camId,
                    localPort: preferredPort,
                    bindHost,
                    urlHost,
                ...
                this.log.info(`Always-on RTSP endpoint for camera ${camId.slice(0, 8)}: ${url} ` +
                    `(Bosch session opens on demand)`);
            }
            catch (err) {
                const msg = err instanceof Error ? err.message : String(err);
                this.log.warn(`Could not start always-on RTSP endpoint for ${camId.slice(0, 8)}: ${msg}`);
            }
        }
    }
    /**
    ...
    async _openPersistentInner(camId) {
        try {
            ...
            const msg = err instanceof Error ? err.message : String(err);
            this.log.debug(`Persistent endpoint ${camId.slice(0, 8)}: ensureLiveSession failed — ${msg}`);
            return null;
        }
    ...
    _armFrontDoorIdle(camId) {
        ...
        const timer = this.setTimeout(() => {
            ...
            void this._reapPersistentIdle(camId).catch((err) => {
                const msg = err instanceof Error ? err.message : String(err);
     ...
     * @param camId  camera UUID
     */
    async _reapPersistentIdle(camId) {
        ...
        this.log.debug(`Persistent endpoint ${camId.slice(0, 8)}: idle for ` +
     ...
     */
    _startStreamIdleReaper() {
        if (this._streamReaperTimer || this.config.stream_idle_reaper !== true) {
            return;
        }
        this.log.info(`Stream idle-reaper enabled (experimental): livestreams with no client for ` +
        ...
        this._streamReaperTimer = this.setInterval(() => {
            void this._streamReaperTick().catch((err) => {
                const msg = err instanceof Error ? err.message : String(err);
     ...
     * @param camId  Camera UUID
     */
    async _pollNotificationTypes(token, camId) {
        try {
            const url = `${auth_1.CLOUD_API}/v11/video_inputs/${camId}/notifications`;
            const resp = await this._httpClient.get(url, {
                headers: { Authorization: `Bearer ${token}`, Accept: "application/json" },
                validateStatus: (s) => (s >= 200 && s < 300) || s === 404 || s === 443,
            });
     ...
     */
    async _pollBatchDLeds(token, cam) {
        const headers = { Authorization: `Bearer ${token}`, Accept: "application/json" };
        const okOr = (s) => (s >= 200 && s < 300) || s === 404 || s === 443;
        const camId = cam.id;
        // timestamp overlay — all cameras
        try {
            const r = await this._httpClient.get(`${auth_1.CLOUD_API}/v11/video_inputs/${camId}/timestamp`, { headers, validateStatus: okOr });
            const d = r.data;
        ...
        if (cam.generation >= 2) {
            try {
                const r = await this._httpClient.get(`${auth_1.CLOUD_API}/v11/video_inputs/${camId}/ledlights`, { headers, validateStatus: okOr });
                const d = r.data;
                if (r.status >= 200 && r.status < 300 && d && typeof d.state === "string") {
                    await this.upsertState(`cameras.${camId}.status_led`, d.state.toUpperCase() === "ON");
                }
            }
            catch {
                /* best-effort */
        ...
        if (cam.hardwareVersion === "HOME_Eyes_Indoor" ||
            cam.hardwareVersion === "CAMERA_INDOOR_GEN2") {
            try {
                const r = await this._httpClient.get(`${auth_1.CLOUD_API}/v11/video_inputs/${camId}/iconLedBrightness`, { headers, validateStatus: okOr });
                const d = r.data;
     ...
     * @param camId Camera UUID
     */
    async _pollOutdoorLighting(token, camId) {
        const headers = { Authorization: `Bearer ${token}`, Accept: "application/json" };
        const okOr = (s) => (s >= 200 && s < 300) || s === 404 || s === 443;
        // /lighting/motion
        try {
            const r = await this._httpClient.get(`${auth_1.CLOUD_API}/v11/video_inputs/${camId}/lighting/motion`, { headers, validateStatus: okOr });
            if (r.status >= 200 && r.status < 300 && r.data) {
                const d = r.data;
                this._motionLightCache.set(camId, { ...d });
                const w = [];
                if (typeof d.lightOnMotionEnabled === "boolean") {
                    w.push(this.upsertState(`cameras.${camId}.motion_light_enabled`, d.lightOnMotionEnabled));
                }
                if (typeof d.motionLightSensitivity === "number") {
                    w.push(this.upsertState(`cameras.${camId}.motion_light_sensitivity`, d.motionLightSensitivity));
                }
                await Promise.all(w);
            }
        }
        ...
        try {
            const r = await this._httpClient.get(`${auth_1.CLOUD_API}/v11/video_inputs/${camId}/lighting/ambient`, { headers, validateStatus: okOr });
            if (r.status >= 200 && r.status < 300 && r.data) {
                const d = r.data;
     ...
     */
    async _pollManagementReads(token, cam) {
        const camId = cam.id;
     ...
     * @param camId Camera UUID
     */
    async _pollFirmwareStatus(token, camId) {
        await this._withCameraLock(this._firmwareLocks, camId, async () => {
            try {
                const resp = await this._httpClient.get(`${auth_1.CLOUD_API}/v11/video_inputs/${camId}/firmware`, {
                    headers: { Authorization: `Bearer ${token}`, Accept: "application/json" },
                ...
                await this.upsertState(`cameras.${camId}.firmware_latest_version`, typeof fw.update === "string" && fw.update ? fw.update : current);
                ...
                this.log.debug(`Firmware status read for ${camId.slice(0, 8)} failed: ` +
                    `${err instanceof Error ? err.message : String(err)}`);
            }
        });
    }
     ...
     */
    async _pollCloudListDp(token, camId, endpoint, dpBase) {
        try {
            const resp = await this._httpClient.get(`${auth_1.CLOUD_API}/v11/video_inputs/${camId}/${endpoint}`, {
                headers: { Authorization: `Bearer ${token}`, Accept: "application/json" },
                validateStatus: (s) => (s >= 200 && s < 300) || s === 404 || s === 442 || s === 443 || s === 444,
            });
            ...
            this.log.debug(`Management read ${endpoint} for ${camId.slice(0, 8)} failed: ` +
                `${err instanceof Error ? err.message : String(err)}`);
        }
    }
    /**
     ...
     * @param camId  Camera UUID
     */
    async _pollLightingSchedule(token, camId) {
        try {
            const resp = await this._httpClient.get(`${auth_1.CLOUD_API}/v11/video_inputs/${camId}/lighting_options`, {
                headers: { Authorization: `Bearer ${token}`, Accept: "application/json" },
                validateStatus: (s) => (s >= 200 && s < 300) || s === 404 || s === 442 || s === 444,
            });
            if (resp.status >= 200 &&
                resp.status < 300 &&
                resp.data &&
            ...
            this.log.debug(`Lighting-schedule read for ${camId.slice(0, 8)} failed: ` +
                `${err instanceof Error ? err.message : String(err)}`);
        }
    }
     ...
     * @param camId Camera UUID
     */
    async _pollCommissioned(token, camId) {
        try {
            const r = await this._httpClient.get(`${auth_1.CLOUD_API}/v11/video_inputs/${camId}/commissioned`, {
                headers: { Authorization: `Bearer ${token}`, Accept: "application/json" },
                validateStatus: (s) => (s >= 200 && s < 300) || s === 404 || s === 443,
            });
            if (r.status < 200 || r.status >= 300 || !r.data) {
            ...
            this.log.debug(`Commissioned poll for ${camId.slice(0, 8)} failed: ` +
                `${err instanceof Error ? err.message : String(err)}`);
        }
    }
    ...
    async _pollAlarmSettings(token, camId) {
        try {
            ...
            if (typeof data.alarmMode === "string") {
                writes.push(this.upsertState(`cameras.${camId}.alarm_mode`, data.alarmMode === "ON"));
            }
            if (typeof data.preAlarmMode === "string") {
                writes.push(this.upsertState(`cameras.${camId}.pre_alarm`, data.preAlarmMode === "ON"));
            }
            await Promise.all(writes);
        }
        catch (err) {
     ...
     * @param camId  Camera UUID
     */
    async _pollAlarmStatus(token, camId) {
        try {
            const url = `${auth_1.CLOUD_API}/v11/video_inputs/${camId}/alarmStatus`;
            const resp = await this._httpClient.get(url, {
                headers: { Authorization: `Bearer ${token}`, Accept: "application/json" },
                validateStatus: (s) => (s >= 200 && s < 300) || s === 404 || s === 443,
            });
            ...
            if (typeof data.intrusionSystem === "string") {
                const writes = [
                    this.upsertState(`cameras.${camId}.alarm_state`, data.intrusionSystem.toLowerCase()),
                    this.upsertState(`cameras.${camId}.alarm_arm`, data.intrusionSystem.toUpperCase() === "ACTIVE"),
                ];
                await Promise.all(writes);
            }
        }
        catch (err) {
            this.log.debug(`Alarm status poll for ${camId.slice(0, 8)} failed: ` +
                `${err instanceof Error ? err.message : String(err)}`);
        }
    }
    /**
            ...
            switch (stateName) {
                ...
                case "status_led": {
                    ...
                    if (!camLed || camLed.generation < 2) {
                        this.log.warn(`status_led write for ${camId.slice(0, 8)} ignored — Gen2 only`);
                        return;
                    }
                ...
                case "power_led_brightness": {
                    ...
                    if (!camPwr ||
                        ...
                        this.log.warn(`power_led_brightness write for ${camId.slice(0, 8)} ignored — Indoor II only`);
                        return;
                    }
                ...
                case "ambient_light_enabled": {
                    ...
                    if (!camOl ||
                        ...
                        this.log.warn(`${stateName} write for ${camId.slice(0, 8)} ignored — Gen2 Outdoor only`);
                        return;
                    }
                    let ok;
                ...
                case "notify_trouble_email": {
                    ...
                    if (!(await this._handleNotificationTypeWrite(camId, apiKey, Boolean(state.val)))) {
                        return; // skip ack — write rejected (privacy 443)
                    }
                    break;
                }
                ...
                case "intercom_enabled": {
                    ...
                    if (!camIc || camIc.generation < 2) {
                        this.log.warn(`intercom_enabled write for ${camId.slice(0, 8)} ignored — Gen2 only`);
                        return; // skip ack
                    }
                ...
                case "pre_alarm": {
                    ...
                    if (!camAl ||
                        ...
                        this.log.warn(`${stateName} write for ${camId.slice(0, 8)} ignored — Indoor II only`);
                        return; // skip ack
                    }
                ...
                case "camera_share": {
                    ...
                    if (!camShare || camShare.generation < 2) {
                        this.log.warn(`camera_share write for ${camId.slice(0, 8)} ignored — Gen2 only`);
                        return;
                    }
                ...
                case "friend_invite": {
                    ...
                    if (!camInv || camInv.generation < 2) {
                        this.log.warn(`friend_invite write for ${camId.slice(0, 8)} ignored — Gen2 only`);
                        return;
                    }
                ...
                case "friend_remove": {
                    ...
                    if (!camRem || camRem.generation < 2) {
                        this.log.warn(`friend_remove write for ${camId.slice(0, 8)} ignored — Gen2 only`);
                        return;
                    }
     ...
     */
    async _handleIntercomWrite(camId, on) {
        if (!this._currentAccessToken) {
            throw new Error("no access token — adapter not ready");
        }
        const url = `${auth_1.CLOUD_API}/v11/video_inputs/${camId}/audio`;
        ...
        this._audioCache.set(camId, { ...audio });
        this.log.info(`Intercom ${on ? "ON" : "OFF"} for ${camId.slice(0, 8)} (HTTP ${resp.status})`);
    }
     ...
     */
    async _putSingleKey(camId, endpoint, body, label) {
        if (!this._currentAccessToken) {
            throw new Error("no access token — adapter not ready");
        }
        const url = `${auth_1.CLOUD_API}/v11/video_inputs/${camId}/${endpoint}`;
        const resp = await this._httpClient.put(url, body, {
            headers: {
                Authorization: `Bearer ${this._currentAccessToken}`,
                "Content-Type": "application/json",
        ...
        this.log.info(`${label} = ${JSON.stringify(body)} for ${camId.slice(0, 8)} (HTTP ${resp.status})`);
        return true;
    }
    /** v1.1.0: status-LED on/off (Gen2). PUT /ledlights {state:"ON"|"OFF"}. */
    _handleStatusLedWrite(camId, on) {
    ...
    _handleTimestampWrite(camId, on) {
    ...
    _handlePowerLedBrightnessWrite(camId, value) {
        const clamped = Math.max(0, Math.min(4, Math.round(value)));
        return this._putSingleKey(camId, "iconLedBrightness", { value: clamped }, "power_led_brightness");
    }
    /**
     ...
     */
    async _putLightingMerge(camId, sub, cache, delta) {
        if (!this._currentAccessToken) {
            throw new Error("no access token — adapter not ready");
        }
        const url = `${auth_1.CLOUD_API}/v11/video_inputs/${camId}/lighting/${sub}`;
        const headers = {
            Authorization: `Bearer ${this._currentAccessToken}`,
            "Content-Type": "application/json",
            Accept: "application/json",
        ...
        if (cached) {
            ...
            if (getResp.status === 443) {
                this.log.warn(`lighting/${sub} write for ${camId.slice(0, 8)} skipped — privacy (443)`);
                return false;
            }
            body = getResp.data ?? {};
        }
        ...
        if (resp.status === 443) {
            this.log.warn(`lighting/${sub} write for ${camId.slice(0, 8)} rejected — privacy (443)`);
            return false;
        }
        cache.set(camId, { ...body });
        this.log.info(`lighting/${sub} ${JSON.stringify(delta)} for ${camId.slice(0, 8)} (HTTP ${resp.status})`);
        return true;
    }
    /** v1.1.0: motion-light on/off + sensitivity (Gen2 Outdoor, /lighting/motion). */
    _handleMotionLightWrite(camId, delta) {
        const body = {};
        if (delta.enabled !== undefined) {
    ...
    _handleAmbientLightWrite(camId, on) {
     ...
     */
    async _handleNotificationTypeWrite(camId, apiKey, on) {
        if (!this._currentAccessToken) {
            throw new Error("no access token — adapter not ready");
        }
        const url = `${auth_1.CLOUD_API}/v11/video_inputs/${camId}/notifications`;
        ...
        if (cached) {
            ...
            if (getResp.status === 443) {
                this.log.warn(`notification-type write for ${camId.slice(0, 8)} skipped — privacy mode (443)`);
                return false;
            }
            body = getResp.data ?? {};
        }
        ...
        if (resp.status === 443) {
            this.log.warn(`notification-type write for ${camId.slice(0, 8)} rejected — privacy mode (443)`);
            return false;
        }
     ...
     */
    _parseZoneArray(camId, raw, dpLabel) {
        let parsed;
        try {
        ...
        if (!Array.isArray(parsed)) {
            this.log.warn(`${dpLabel} write for ${camId.slice(0, 8)} ignored — expected a JSON array`);
            return null;
        }
        ...
        for (const entry of parsed) {
            if (typeof entry !== "object" ||
                ...
                this.log.warn(`${dpLabel} write for ${camId.slice(0, 8)} ignored — every entry needs numeric x,y,w,h in 0.0-1.0`);
                return null;
            }
        }
        return parsed;
     ...
     */
    async _postZoneArray(camId, endpoint, zones, dpLabel) {
        if (!this._currentAccessToken) {
            throw new Error("no access token — adapter not ready");
        }
        const url = `${auth_1.CLOUD_API}/v11/video_inputs/${camId}/${endpoint}`;
        const resp = await this._httpClient.post(url, zones, {
            headers: {
                Authorization: `Bearer ${this._currentAccessToken}`,
                "Content-Type": "application/json",
                Accept: "application/json",
        ...
        if (resp.status === 443) {
            this.log.warn(`${dpLabel} write for ${camId.slice(0, 8)} rejected — camera in privacy mode (HTTP 443)`);
            return false;
        }
        ...
        this.log.info(`${dpLabel} for ${camId.slice(0, 8)}: ${zones.length} entries (HTTP ${resp.status})`);
        return true;
    }
    /**
     ...
     */
    async _handleMotionZonesWrite(camId, raw) {
     ...
     */
    async _handlePrivacyMasksWrite(camId, raw) {
     ...
     */
    async _handleRuleCreate(camId, raw) {
        if (!this._currentAccessToken) {
            throw new Error("no access token — adapter not ready");
        }
        let req;
        ...
        if (typeof req !== "object" ||
            ...
            this.log.warn(`rule_create write for ${camId.slice(0, 8)} ignored — expected {name,isActive,startTime,endTime,weekdays}`);
            return false;
        }
        const body = {
            id: null,
        ...
        const resp = await this._httpClient.post(`${auth_1.CLOUD_API}/v11/video_inputs/${camId}/rules`, body, {
            headers: {
                Authorization: `Bearer ${this._currentAccessToken}`,
                "Content-Type": "application/json",
                Accept: "application/json",
        ...
        if (resp.status === 443) {
            this.log.warn(`rule_create write for ${camId.slice(0, 8)} rejected — camera in privacy mode (HTTP 443)`);
            return false;
        }
     ...
     */
    async _handleRuleUpdate(camId, raw) {
        if (!this._currentAccessToken) {
            throw new Error("no access token — adapter not ready");
        }
        let req;
        ...
        if (typeof req !== "object" || req === null || typeof req.id !== "string") {
            this.log.warn(`rule_update write for ${camId.slice(0, 8)} ignored — expected {"id":"...",...}`);
            return false;
        }
        const listUrl = `${auth_1.CLOUD_API}/v11/video_inputs/${camId}/rules`;
        const headers = {
            Authorization: `Bearer ${this._currentAccessToken}`,
            "Content-Type": "application/json",
            Accept: "application/json",
        ...
        if (listResp.status === 443) {
            this.log.warn(`rule_update write for ${camId.slice(0, 8)} rejected — camera in privacy mode (HTTP 443)`);
            return false;
        }
        ...
        if (!existing) {
            this.log.warn(`rule_update write for ${camId.slice(0, 8)} ignored — rule id "${String(req.id)}" not found`);
            return false;
        }
        ...
        const resp = await this._httpClient.put(`${listUrl}/${encodeURIComponent(String(req.id))}`, merged, {
            headers,
        ...
        if (resp.status === 404) {
            this.log.warn(`rule_update write for ${camId.slice(0, 8)} — rule "${String(req.id)}" not found (deleted concurrently?)`);
            return false;
        }
        this.log.info(`Rule ${String(req.id)} updated for ${camId.slice(0, 8)} (HTTP ${resp.status})`);
        await this._pollCloudListDp(this._currentAccessToken, camId, "rules", "rules");
        return true;
    }
    /**
     ...
     */
    async _handleRuleDelete(camId, ruleId) {
        if (!this._currentAccessToken) {
            throw new Error("no access token — adapter not ready");
        }
        if (!ruleId) {
        ...
        const resp = await this._httpClient.delete(
        // encodeURIComponent (bug-hunt round 1+2): ruleId is raw
        // user-writable input reaching a URL path segment.
        `${auth_1.CLOUD_API}/v11/video_inputs/${camId}/rules/${encodeURIComponent(ruleId)}`, {
            headers: {
                Authorization: `Bearer ${this._currentAccessToken}`,
                Accept: "application/json",
            },
        ...
        if (resp.status === 443) {
            this.log.warn(`rule_delete write for ${camId.slice(0, 8)} rejected — camera in privacy mode (HTTP 443)`);
            return false;
        }
        if (resp.status === 404) {
            this.log.warn(`rule_delete write for ${camId.slice(0, 8)} — rule "${ruleId}" not found`);
            return false;
        }
     ...
     */
    async _handleCameraShare(camId, raw) {
        if (!this._currentAccessToken) {
            throw new Error("no access token — adapter not ready");
        }
        let req;
        ...
        if (typeof req !== "object" ||
            ...
            !req.friendId) {
            this.log.warn(`camera_share write for ${camId.slice(0, 8)} ignored — expected {"friendId":"..."}`);
            return false;
        }
        const entry = { videoInputId: camId };
        ...
        // friend_remove).
        const resp = await this._httpClient.put(`${auth_1.CLOUD_API}/v11/friends/${encodeURIComponent(req.friendId)}/share`, [entry], {
            headers: {
                Authorization: `Bearer ${this._currentAccessToken}`,
                "Content-Type": "application/json",
                Accept: "application/json",
        ...
        if (resp.status === 443) {
            this.log.warn(`camera_share write for ${camId.slice(0, 8)} rejected — camera in privacy mode (HTTP 443)`);
            return false;
        }
        this.log.info(`Camera ${camId.slice(0, 8)} shared with friend ${String(req.friendId).slice(0, 8)} (HTTP ${resp.status})`);
        await this._pollCloudListDp(this._currentAccessToken, camId, "shared_with_friends", "shared_with_friends");
        return true;
    }
    /**
     ...
     */
    async _handleFriendInvite(camId, email) {
        if (!this._currentAccessToken) {
            throw new Error("no access token — adapter not ready");
        }
        const trimmed = email.trim();
        if (!trimmed.includes("@")) {
            this.log.warn(`friend_invite write for ${camId.slice(0, 8)} ignored — not an email address`);
            return false;
        }
        const resp = await this._httpClient.post(`${auth_1.CLOUD_API}/v11/friends`, { invitationEmail: trimmed, nickName: trimmed }, {
            headers: {
                Authorization: `Bearer ${this._currentAccessToken}`,
                "Content-Type": "application/json",
                Accept: "application/json",
     ...
     */
    async _handleFriendRemove(camId, friendId) {
        if (!this._currentAccessToken) {
            throw new Error("no access token — adapter not ready");
        }
        if (!friendId) {
        ...
        // user-writable input reaching a URL path segment.
        const resp = await this._httpClient.delete(`${auth_1.CLOUD_API}/v11/friends/${encodeURIComponent(friendId)}`, {
            headers: {
                Authorization: `Bearer ${this._currentAccessToken}`,
                Accept: "application/json",
            },
     ...
     * @param camId Camera UUID
     */
    async _handleFirmwareInstall(camId) {
        if (!this._currentAccessToken) {
            throw new Error("no access token — adapter not ready");
        }
        return this._withCameraLock(this._firmwareLocks, camId, async () => {
            ...
            if (fw.updating === true) {
                this.log.warn(`firmware_install for ${camId.slice(0, 8)} ignored — install already in progress`);
                return false;
            }
            const target = typeof fw.update === "string" ? fw.update : "";
            if (!target) {
                this.log.warn(`firmware_install for ${camId.slice(0, 8)} ignored — no update currently available`);
                return false;
            }
            const resp = await this._httpClient.put(`${auth_1.CLOUD_API}/v11/video_inputs/${camId}/firmware`, { id: target }, {
                headers: {
                    Authorization: `Bearer ${this._currentAccessToken}`,
                    "Content-Type": "application/json",
                    Accept: "application/json",
            ...
            if (!ok) {
                this.log.warn(`firmware_install for ${camId.slice(0, 8)} rejected by Bosch cloud (HTTP ${resp.status})`);
                return false;
            }
            fw.updating = true;
            this._firmwareCache.set(camId, fw);
            await this.upsertState(`cameras.${camId}.firmware_updating`, true);
            this.log.info(`Firmware install to ${target} triggered for ${camId.slice(0, 8)} (HTTP ${resp.status})`);
            return true;
        });
    }
     ...
     */
    async _handleAlarmArmWrite(camId, arm) {
        if (!this._currentAccessToken) {
            throw new Error("no access token — adapter not ready");
        }
        const url = `${auth_1.CLOUD_API}/v11/video_inputs/${camId}/intrusionSystem/arming`;
        const resp = await this._httpClient.put(url, { arm }, {
            headers: {
                Authorization: `Bearer ${this._currentAccessToken}`,
                "Content-Type": "application/json",
                Accept: "application/json",
        ...
        this.log.info(`Alarm system ${arm ? "ARMED" : "DISARMED"} for ${camId.slice(0, 8)} (HTTP ${resp.status})`);
    }
    /**
     ...
     */
    async handleFrontLightIntensityUpdate(camId, brightness) {
        const cam = this._cameras.get(camId);
        if (!cam || cam.generation < 2 || cam.featureLight !== true) {
            this.log.warn(`front_light_intensity write for ${camId.slice(0, 8)} ignored — Gen2 lighting not supported`);
        ...
        const next = (0, alarm_light_1.buildFrontLightUpdate)(current, safeVal);
        const result = await (0, alarm_light_1.putLightingState)(this._httpClient, this._currentAccessToken, camId, next);
        if (!result) {
            throw new Error(`PUT /lighting/switch returned non-success for ${camId.slice(0, 8)}`);
        }
     ...
     */
    _logLivestreamHintIfAllOff(cameras) {
        if (cameras.length === 0) {
            return false;
        }
        ...
        let hint = `Livestream is opt-in and OFF by default → cameras.<id>.stream_url is empty and go2rtc/recorders ` +
    ...
    async _fetchSnapJpgWithRetry(camId, snapUrl, session) {
        ...
        for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
            try {
                ...
                this.log.silly(`Snapshot retry ${attempt}/${MAX_ATTEMPTS - 1} for ${camId.slice(0, 8)}: ${msg}`);
     ...
     */
    async _resolveCameraStatus(camId) {
        ...
        try {
            const pr = await this._httpClient.get(`${auth_1.CLOUD_API}/v11/video_inputs/${camId}/ping`, {
                headers: { Authorization: `Bearer ${token}`, Accept: "application/json" },
                validateStatus: () => true,
            });
            ...
            this.log.debug(`[status] /ping for ${camId.slice(0, 8)} failed: ` +
                `${err instanceof Error ? err.message : String(err)}`);
        }
        // Fallback 2: cloud /commissioned → {connected, commissioned, configured}.
        try {
            const cr = await this._httpClient.get(`${auth_1.CLOUD_API}/v11/video_inputs/${camId}/commissioned`, {
                headers: { Authorization: `Bearer ${token}`, Accept: "application/json" },
                validateStatus: () => true,
            });
            if (cr.status === 444) {
            ...
            this.log.debug(`[status] /commissioned for ${camId.slice(0, 8)} failed: ` +
                `${err instanceof Error ? err.message : String(err)}`);
        }
        return "UNKNOWN";
    }
    ...
    async _handleSessionLimitError(camId) {
        ...
        if (status === "OFFLINE" || status === "UPDATING") {
            this.log.debug(`[session-quota] Camera ${camId.slice(0, 8)} is ${status} — not a quota ` +
        ...
        if (hits.length === 1) {
            this.log.warn(`[session-quota] Bosch returned HTTP 444 for camera ${camPrefix} — too many ` +
                `simultaneous live sessions (shared 3-session limit). Will auto-retry.`);
        }
        else {
            this.log.debug(`[session-quota] HTTP 444 for ${camPrefix} — hit ${hits.length} in the 5-min window.`);
        }
        ...
        if (hits.length === threshold) {
            this.log.warn(`[session-quota] ${threshold}+ session-quota hits in 5 min for camera ` +
        ...
        if (hits.length >= BoschSmartHomeCamera.MAX_SESSION_RETRIES) {
            this.log.info(`[session-quota] Auto-retry paused for ${camPrefix} after ${hits.length} attempts ` +
        ...
        try {
            ...
            if (priv?.val === true) {
                ...
                try {
                    ...
                    this.log.debug(`[status] cloud reconcile for ${camId.slice(0, 8)} failed (non-fatal): ` +
                        `${err instanceof Error ? err.message : String(err)}`);
                }
                return;
            }
     ...
     * @param camId Camera UUID
     */
    async _reconcileOnlineViaCloud(camId) {
        const now = Date.now();
