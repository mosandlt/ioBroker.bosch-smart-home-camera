...
import { fetchCameras, type BoschCamera, UnauthorizedError, sanitizeId } from "./lib/cameras";
...
import {
    ...
    buildLedGroupBrightnessUpdate,
    buildFrontLightWhiteBalanceUpdate,
...
import { validateAiEndpointUrl } from "./lib/ssrf_guard";

import { MqttBridge } from "./lib/mqtt_bridge";

import { fetchRcpLan, parseOnvifScopes, formatRcpVersion } from "./lib/rcp_lan_helper";

import { fetchFeatureFlags } from "./lib/cloud_feature_flags";
...
class BoschSmartHomeCamera extends utils.Adapter {
    ...
    private _webStream: WebStreamManager | null = null;
    ...
    private _mjpegFailCount: Map<string, number> = new Map();
    private static readonly MJPEG_FASTPATH_MAX_FAILS = 2;
     ...
     */
    private _unsupportedFeatures: Map<string, Set<string>> = new Map();
     ...
     */
    private _pollBackoff: Map<string, { failCount: number; nextAttempt: number }> = new Map();
    private static readonly POLL_BACKOFF_BASE_MS = 30_000;
    private static readonly POLL_BACKOFF_CAP_MS = 300_000;
     ...
     */
    private _lastEventFetchAt = -Infinity;
    /**
     ...
     */
    private _persistentInflight = new Map<string, Promise<number | null>>();
     ...
     */
    private _snapshotInflight = new Map<
        string,
        { promise: Promise<void>; asMotionEvent: boolean }
    >();
     ...
     */
    private static readonly STATE_POLL_INTERVAL_MS = 60_000;
     ...
     */
    private _maintenanceNotifiedKey: [string, string] | null = null;
     ...
     */
    private _lastCameraStatus: Record<string, string> = {};
    ...
    private _streamGeneration = new Map<string, number>();
    ...
    private _globalLightingCache = new Map<string, Record<string, unknown>>();
    ...
    private _hardResetConfirmSetAt = new Map<string, number>();

    /** 60s confirm window for hard_reset — matches the DP's own description text. */
    private static readonly HARD_RESET_CONFIRM_WINDOW_MS = 60_000;
    ...
    private _aiAnalyzeInflight = new Map<string, Promise<void>>();
    ...
    private _motionCache = new Map<string, Record<string, unknown>>();
    ...
    private _audioDetectionCache = new Map<string, Record<string, unknown>>();
     ...
     */
    private _mqttBridge: MqttBridge | null = null;
     ...
     */
    private get _motionActiveWindowMs(): number {
    ...
    private onUiClientSubscribe(info: { clientId: string; message?: ioBroker.Message }): {
        ...
        const body = info.message?.message as
            | { type?: string; data?: { width?: number } }
            | undefined;
        ...
        const ok =
            this._webStream?.addViewer(info.clientId, camId, body?.data?.width ?? 0) ?? false;
        if (!ok) {
     ...
     */
    private onUiClientUnsubscribe(info: { clientId: string; message?: ioBroker.Message }): void {
        const body = info.message?.message as { type?: string | string[] } | undefined;
     ...
     */
    private _resolveWebStreamUrl(camId: string): string | null {
     ...
     */
    private _resolveCameraId(requested: string): string | null {
     ...
     */
    private async _maybeAnnounceMaintenanceState(
        ...
        const notifyKey: [string, string] = [mw.link, state];
        if (
            this._maintenanceNotifiedKey !== null &&
     ...
     */
    private async _migrateStateRoles(): Promise<void> {
        ...
        try {
            ...
            this.log.warn(
                `v1.2.6 role migration failed: ${err instanceof Error ? err.message : String(err)}`,
            );
        }
        if (fixed > 0) {
    ...
    private async ensureCameraObjects(cameras: BoschCamera[]): Promise<void> {
        ...
        for (const cam of cameras) {
            ...
            await this.setObjectNotExistsAsync(`${prefix}.hard_reset_confirm`, {
                ...
                common: {
                    name:
                        "Type the camera's exact current title here, THEN write true to " +
            ...
            await this.setObjectNotExistsAsync(`${prefix}.hard_reset`, {
                ...
                common: {
                    name:
                        "DESTRUCTIVE: factory-reset camera (write true to trigger). " +
            ...
            await this.setObjectNotExistsAsync(`${prefix}.ai_analyze`, {
                ...
                common: {
                    name:
                        "Trigger AI analysis of the latest snapshot (write true) — " +
            ...
            await this.upsertState(
                `${prefix}.unread_events_count`,
                typeof unreadState?.val === "number" ? unreadState.val : 0,
            );
            ...
            for (const dp of ["events_today", "movement_count", "audio_count"] as const) {
                const cached = await this.getStateAsync(`${prefix}.${dp}`);
                await this.upsertState(
                    `${prefix}.${dp}`,
                    typeof cached?.val === "number" ? cached.val : 0,
                );
            }
     ...
     */
    private async ensureCloudObjects(): Promise<void> {
     ...
     * @param camId  Camera UUID
     */
    private async _attemptBackoffRenewal(
        camId: string,
        ...
        if (
            ...
            this.log.debug(
                `RTSP watchdog: skipping stale renewal for ${camPrefix} — stream was torn down`,
            );
            return;
        }
        ...
        try {
            ...
            if (
                ...
                this.log.debug(
                    `RTSP watchdog: stream for ${camPrefix} torn down during renewal — discarding new session`,
                );
                if (this._currentAccessToken) {
                    void closeLiveSession(this._httpClient, this._currentAccessToken, camId).catch(
                        () => undefined,
                    );
                }
                return;
     ...
     */
    private async _publishStreamParts(camId: string, url: string): Promise<void> {
        let host = "";
        let port = 0;
        let path = "";
        if (url) {
     ...
     * @param camId  Camera UUID
     */
    private async _openEmergencySession(
        camId: string,
        ...
        try {
            ...
            this._sessionStartTime.set(camId, Date.now());
            this.log.info(
                `Emergency LiveSession opened for LAN-RCP write on camera ${camId.slice(0, 8)}`,
            );
    ...
    private async onReady(): Promise<void> {
        ...
        this._webStream = new WebStreamManager({
        ...
        if (this.config.startup_snapshot === true) {
            ...
            for (const cam of cameras) {
                void this._reconcileOnlineViaCloud(cam.id).catch((err: unknown) => {
                    const msg = err instanceof Error ? err.message : String(err);
        ...
        this._fcmListener.on("motion", (ev: FcmEventPayload) => {
            void this.onFcmEvent(ev).catch((err: unknown) => {
                const msg = err instanceof Error ? err.message : String(err);
        ...
        this._fcmListener.on("error-logged", (msg: string) => {
        ...
        if (this.config.mqtt_enabled) {
            this._mqttBridge = new MqttBridge(this.config, this.log);
            void this._mqttBridge.connect().catch((err: unknown) => {
                this.log.warn(
                    `MQTT Bridge: initial connect failed — ${
                        err instanceof Error ? err.message : String(err)
                    }`,
                );
            });
        ...
        if (snapPort > 0) {
            this._snapshotHost = detectLanIp();
            try {
                this._snapshotServer = await startSnapshotServer({
                    port: snapPort,
                ...
                for (const camId of this._cameras.keys()) {
                    await this.setStateAsync(
                        `cameras.${camId}.snapshot_url`,
                        snapshotUrl(this._snapshotHost, snapPort, camId),
                        true,
                    );
                }
            } catch (err: unknown) {
                this.log.warn(
                    `snapshot server disabled — ${err instanceof Error ? err.message : String(err)}`,
                );
            }
        }
    ...
    private async _openPersistentInner(camId: string): Promise<number | null> {
        try {
    ...
    private async _pollCameraStateOnce(): Promise<void> {
        ...
        // don't race.
        await Promise.all(
            cameras.map((cam) => this._pollSingleCameraState(token, cam, doSlowTier)),
        );
    }
     ...
     * @param doSlowTier
     */
    private async _pollSingleCameraState(
        token: string,
        cam: BoschCamera,
        doSlowTier = false,
    ): Promise<void> {
        ...
        if (cam.privacyMode !== undefined) {
            ...
            if (current?.val !== desired) {
                ...
                if (this._liveSessions.has(cam.id)) {
                    ...
                    this._streamGeneration.set(
                        cam.id,
                        (this._streamGeneration.get(cam.id) ?? 0) + 1,
                    );
                    ...
                    this.log.info(
                        `Privacy toggled externally for ${cam.id.slice(0, 8)} — ` +
                ...
                if (desired === false && this._livestreamEnabled.get(cam.id) === true) {
                    void this.ensureLiveSession(cam.id).catch((err: unknown) => {
                        const msg = err instanceof Error ? err.message : String(err);
                        this.log.debug(
                            `Eager LiveSession refresh after privacy ON→OFF failed for ` +
        ...
        if (curBotBr?.val !== bot.brightness) {
            writes.push(
                this.upsertState(`cameras.${cam.id}.bottom_led_brightness`, bot.brightness),
            );
        }
        const frontWhiteBalance = ls.frontLightSettings.whiteBalance ?? -1;
        if (curWhiteBalance?.val !== frontWhiteBalance) {
            writes.push(
                this.upsertState(`cameras.${cam.id}.front_light_white_balance`, frontWhiteBalance),
            );
        }
    ...
    private async _pollIntrusionConfig(token: string, camId: string): Promise<void> {
        try {
            const url = `${CLOUD_API}/v11/video_inputs/${camId}/intrusionDetectionConfig`;
     ...
     * @param camId  Camera UUID
     */
    private async _pollMotionConfig(token: string, camId: string): Promise<void> {
        try {
            const url = `${CLOUD_API}/v11/video_inputs/${camId}/motion`;
            const resp = await this._httpClient.get<unknown>(url, {
                headers: { Authorization: `Bearer ${token}`, Accept: "application/json" },
            ...
            this.log.debug(
                `Motion config poll for ${camId.slice(0, 8)} failed: ` +
                    `${err instanceof Error ? err.message : String(err)}`,
            );
        }
    }
     ...
     * @param camId  Camera UUID
     */
    private async _pollRecordingOptions(token: string, camId: string): Promise<void> {
        try {
            const url = `${CLOUD_API}/v11/video_inputs/${camId}/recording_options`;
            const resp = await this._httpClient.get<unknown>(url, {
                headers: { Authorization: `Bearer ${token}`, Accept: "application/json" },
                validateStatus: (s) => (s >= 200 && s < 300) || s === 404 || s === 443,
            });
     ...
     * @param camId Camera UUID
     */
    private async _pollAudioDetectionConfig(token: string, camId: string): Promise<void> {
        try {
            const url = `${CLOUD_API}/v11/video_inputs/${camId}/audioDetectionConfig`;
            const resp = await this._httpClient.get<unknown>(url, {
                headers: { Authorization: `Bearer ${token}`, Accept: "application/json" },
                validateStatus: (s) => (s >= 200 && s < 300) || s === 404 || s === 443,
            });
            ...
            if (typeof data.detectGlassBreak === "boolean") {
                writes.push(
                    this.upsertState(
                        `cameras.${camId}.glass_break_detection`,
                        data.detectGlassBreak,
                    ),
                );
            }
            if (typeof data.detectFireAlarm === "boolean") {
                writes.push(
                    this.upsertState(`cameras.${camId}.fire_alarm_detection`, data.detectFireAlarm),
                );
            }
            await Promise.all(writes);
        } catch (err: unknown) {
            this.log.debug(
                `Audio-detection poll for ${camId.slice(0, 8)} failed: ` +
                    `${err instanceof Error ? err.message : String(err)}`,
            );
        }
    }
        ...
        try {
            ...
            if (r.status >= 200 && r.status < 300 && r.data) {
                ...
                if (typeof sched === "string") {
                    ...
                    const t = (sched as Record<string, unknown>).type;
     ...
     * @param camId  Camera UUID
     */
    private async _pollLensElevation(token: string, camId: string): Promise<void> {
        try {
            const url = `${CLOUD_API}/v11/video_inputs/${camId}/lens_elevation`;
            const resp = await this._httpClient.get<unknown>(url, {
                headers: { Authorization: `Bearer ${token}`, Accept: "application/json" },
                validateStatus: (s) => (s >= 200 && s < 300) || s === 404 || s === 443,
            });
            ...
            this.log.debug(
                `Lens elevation poll for ${camId.slice(0, 8)} failed: ` +
                    `${err instanceof Error ? err.message : String(err)}`,
            );
        }
    }
     ...
     * @param camId  Camera UUID
     */
    private async _pollGlobalLighting(token: string, camId: string): Promise<void> {
        try {
            const url = `${CLOUD_API}/v11/video_inputs/${camId}/lighting`;
            const resp = await this._httpClient.get<unknown>(url, {
                headers: { Authorization: `Bearer ${token}`, Accept: "application/json" },
                validateStatus: (s) => (s >= 200 && s < 300) || s === 404 || s === 443,
            });
            ...
            const dt =
                typeof data.darknessThreshold === "number" ? data.darknessThreshold : undefined;
            if (dt !== undefined) {
            ...
            this.log.debug(
                `Global lighting poll for ${camId.slice(0, 8)} failed: ` +
                    `${err instanceof Error ? err.message : String(err)}`,
            );
        }
    }
     ...
     * @param camId  Camera UUID
     */
    private async _pollAlarmSettings(token: string, camId: string): Promise<void> {
        try {
            const url = `${CLOUD_API}/v11/video_inputs/${camId}/alarm_settings`;
            const resp = await this._httpClient.get<unknown>(url, {
                headers: { Authorization: `Bearer ${token}`, Accept: "application/json" },
                validateStatus: (s) => (s >= 200 && s < 300) || s === 404 || s === 443,
            });
            ...
            const writes: Promise<void>[] = [];
            const sirenDur =
                typeof data.alarmDelayInSeconds === "number" ? data.alarmDelayInSeconds : undefined;
            const actDelay =
                typeof data.alarmActivationDelaySeconds === "number"
                    ? data.alarmActivationDelaySeconds
                    : undefined;
            const preDelay =
                typeof data.preAlarmDelayInSeconds === "number"
     ...
     * @param camId  Camera UUID
     */
    private async _pollLanDiagnostics(camId: string): Promise<void> {
        ...
        try {
            const raw = await fetchRcpLan(session, "0x0a98");
            if (raw) {
                const scopes = parseOnvifScopes(raw);
                await this.upsertState(`cameras.${camId}.onvif_scopes`, JSON.stringify(scopes));
                this.log.debug(
                    `F4 ONVIF scopes for ${camId.slice(0, 8)}: ` +
                        `name="${scopes.name}" hw="${scopes.hardware}" profiles=${JSON.stringify(scopes.profiles)}`,
                );
            }
        } catch (err: unknown) {
            this.log.debug(
                `F4 ONVIF scopes poll error for ${camId.slice(0, 8)}: ` +
                    `${err instanceof Error ? err.message : String(err)}`,
            );
        }
        ...
        try {
            const raw = await fetchRcpLan(session, "0xff00");
            if (raw && raw.length >= 4) {
                const ver = formatRcpVersion(raw);
                if (ver) {
            ...
            this.log.debug(
                `F6 RCP version poll error for ${camId.slice(0, 8)}: ` +
                    `${err instanceof Error ? err.message : String(err)}`,
            );
        }
    }
     ...
     * @param token  Current access_token
     */
    private async _pollFeatureFlags(token: string): Promise<void> {
        try {
            const result = await fetchFeatureFlags(this._httpClient, token);
            if (!result) {
                return;
            }
            await Promise.all([
    ...
    private _isFeatureUnsupported(camId: string, feature: string): boolean {
    ...
    private _markFeatureUnsupported(camId: string, feature: string): void {
    ...
    private _backoffKey(camId: string, endpoint: string): string {
        return `${camId}:${endpoint}`;
    }

    /** v0.9.1 — return true if this poll should be skipped due to backoff. */
    private _shouldSkipPoll(camId: string, endpoint: string): boolean {
     ...
     */
    private _recordPollResult(camId: string, endpoint: string, success: boolean): void {
        ...
        const delay = Math.min(
            BoschSmartHomeCamera.POLL_BACKOFF_BASE_MS * Math.pow(2, failCount - 1),
            BoschSmartHomeCamera.POLL_BACKOFF_CAP_MS,
        );
        this._pollBackoff.set(key, { failCount, nextAttempt: Date.now() + delay });
    }

    private async _pollWifiInfo(token: string, camId: string): Promise<void> {
        // v0.9.1: skip when backoff window is open (camera returning consistent 444).
        if (this._shouldSkipPoll(camId, "wifiinfo")) {
            return;
        }
        try {
            const url = `${CLOUD_API}/v11/video_inputs/${camId}/wifiinfo`;
            const resp = await this._httpClient.get<unknown>(url, {
                headers: { Authorization: `Bearer ${token}`, Accept: "application/json" },
            ...
            this._recordPollResult(camId, "wifiinfo", false);
            this.log.debug(
                `WiFi info poll for ${camId.slice(0, 8)} failed: ` +
                    `${err instanceof Error ? err.message : String(err)}`,
            );
        }
    }
     ...
     */
    private async _pollUnreadCount(token: string, camId: string): Promise<void> {
        if (this._shouldSkipPoll(camId, "unread_events")) {
            return;
        }
        try {
            const url = `${CLOUD_API}/v11/events?videoInputId=${camId}&limit=50`;
            const resp = await this._httpClient.get<unknown>(url, {
                headers: { Authorization: `Bearer ${token}`, Accept: "application/json" },
            ...
            this._recordPollResult(camId, "unread_events", false);
            this.log.debug(
                `Unread events poll for ${camId.slice(0, 8)} failed: ` +
                    `${err instanceof Error ? err.message : String(err)}`,
            );
        }
    }
        ...
        try {
            switch (stateName) {
                ...
                case "motion_sensitivity": {
                    ...
                    if (!allowed.has(sens)) {
                        this.log.warn(
                            `motion_sensitivity write for ${camId.slice(0, 8)} ignored — invalid value "${sens}"`,
                        );
                ...
                case "detection_mode": {
                    ...
                    if (!camDm || camDm.generation < 2) {
                        this.log.warn(
                            `detection_mode write for ${camId.slice(0, 8)} ignored — Gen2 only`,
                        );
                    ...
                    if (!dmAllowed.has(dm)) {
                        this.log.warn(
                            `detection_mode write for ${camId.slice(0, 8)} ignored — invalid value "${dm}"`,
                        );
                ...
                case "top_led_brightness":
                    await this.handleLedGroupBrightnessUpdate(
                        camId,
                        "topLedLightSettings",
                        typeof state.val === "number" ? state.val : parseInt(String(state.val), 10),
                    );
                ...
                case "front_light_white_balance":
                    await this.handleFrontLightWhiteBalanceUpdate(
                        camId,
                        typeof state.val === "number" ? state.val : parseFloat(String(state.val)),
                    );
                    return; // see top_led_brightness comment above
                ...
                case "rename": {
                    ...
                    if (!newTitle) {
                        this.log.warn(
                            `rename write for ${camId.slice(0, 8)} ignored — empty title`,
                        );
                        return; // skip ack — keep last valid value
                    }
                ...
                case "microphone_level": {
                    ...
                    if (!camAudio || camAudio.generation < 2) {
                        this.log.warn(
                            `microphone_level write for ${camId.slice(0, 8)} ignored — Gen2 only`,
                        );
                ...
                case "speaker_level": {
                    ...
                    if (!camSpk || camSpk.generation < 2) {
                        this.log.warn(
                            `speaker_level write for ${camId.slice(0, 8)} ignored — Gen2 only`,
                        );
                        return; // skip ack
                    }
                ...
                case "intrusion_sensitivity": {
                    ...
                    if (!camIs || camIs.generation < 2) {
                        this.log.warn(
                            `intrusion_sensitivity write for ${camId.slice(0, 8)} ignored — Gen2 only`,
                        );
                        return; // skip ack
                    }
                    {
                ...
                case "intrusion_distance": {
                    ...
                    if (!camId2 || camId2.generation < 2) {
                        this.log.warn(
                            `intrusion_distance write for ${camId.slice(0, 8)} ignored — Gen2 only`,
                        );
                        return; // skip ack
                    }
                    {
                ...
                case "glass_break_detection": {
                    ...
                    if (!camGb || camGb.generation < 2 || camGb.featureSound !== true) {
                        this.log.warn(
                            `glass_break_detection write for ${camId.slice(0, 8)} ignored — Gen2 Audio-Plus only`,
                        );
                        return; // skip ack
                    }
                    if (
                        !(await this._handleAudioDetectionWrite(
                            camId,
                            "detectGlassBreak",
                            Boolean(state.val),
                        ))
                ...
                case "fire_alarm_detection": {
                    ...
                    if (!camFa || camFa.generation < 2 || camFa.featureSound !== true) {
                        this.log.warn(
                            `fire_alarm_detection write for ${camId.slice(0, 8)} ignored — Gen2 Audio-Plus only`,
                        );
                        return; // skip ack
                    }
                    if (
                ...
                case "pan_position": {
                    ...
                    if (!camPan || camPan.panLimit <= 0) {
                        this.log.warn(
                            `pan_position write for ${camId.slice(0, 8)} ignored — camera has no pan hardware`,
                        );
                        return; // skip ack
                    }
                    {
                ...
                case "pan_preset": {
                    ...
                    if (!camPreset || camPreset.panLimit <= 0) {
                        this.log.warn(
                            `pan_preset write for ${camId.slice(0, 8)} ignored — camera has no pan hardware`,
                        );
                        return; // skip ack
                    }
                    ...
                    if (!(presetName in PAN_PRESET_MAP)) {
                        this.log.warn(
                            `pan_preset write for ${camId.slice(0, 8)}: unknown preset "${presetName}" — ignored`,
                        );
                        return; // skip ack
                    }
                ...
                case "lens_elevation": {
                    ...
                    if (!camLe || camLe.generation < 2) {
                        this.log.warn(
                            `lens_elevation write for ${camId.slice(0, 8)} ignored — Gen2 only`,
                        );
                        return; // skip ack
                    }
                ...
                case "darkness_threshold": {
                    ...
                    if (
                        ...
                        this.log.warn(
                            `darkness_threshold write for ${camId.slice(0, 8)} ignored — Gen2 Outdoor only`,
                        );
                        return; // skip ack
                    }
                ...
                case "siren_duration": {
                    ...
                    if (
                        ...
                        this.log.warn(
                            `siren_duration write for ${camId.slice(0, 8)} ignored — Indoor II only`,
                        );
                        return; // skip ack
                    }
                ...
                case "alarm_activation_delay": {
                    ...
                    if (
                        ...
                        this.log.warn(
                            `alarm_activation_delay write for ${camId.slice(0, 8)} ignored — Indoor II only`,
                        );
                        return; // skip ack
                    }
                    await this._handleAlarmSettingsWrite(camId, {
                ...
                case "pre_alarm_delay": {
                    ...
                    if (
                        ...
                        this.log.warn(
                            `pre_alarm_delay write for ${camId.slice(0, 8)} ignored — Indoor II only`,
                        );
                        return; // skip ack
                    }
                    await this._handleAlarmSettingsWrite(camId, {
                ...
                case "autofollow_enabled": {
                    ...
                    if (!camAf || camAf.panLimit <= 0) {
                        this.log.warn(
                            `autofollow_enabled write for ${camId.slice(0, 8)} ignored — Gen1 360° only`,
                        );
                        return; // skip ack
                    }
     ...
     * @param level  0–100
     */
    private async _handleAudioLevelWrite(
        camId: string,
        field: "microphone" | "speaker",
        level: number,
    ): Promise<void> {
        if (!this._currentAccessToken) {
     ...
     */
    private async _handleNotificationsWrite(camId: string, enabled: boolean): Promise<boolean> {
        if (!this._currentAccessToken) {
            throw new Error("no access token — adapter not ready");
        }
        const url = `${CLOUD_API}/v11/video_inputs/${camId}/enable_notifications`;
        const body = {
        ...
        if (resp.status === 443) {
            this.log.warn(
                `notifications write for ${camId.slice(0, 8)} rejected — camera in privacy mode (HTTP 443)`,
            );
            return false; // skip ack — DP keeps its real value, poll re-syncs
        }
        this.log.info(
            `Notifications ${enabled ? "ON" : "OFF"} for ${camId.slice(0, 8)} (HTTP ${resp.status})`,
        );
        return true;
    }
     ...
     */
    private async _handleMotionWrite(
        camId: string,
        delta: { enabled?: boolean; sensitivity?: string },
    ): Promise<boolean> {
        if (!this._currentAccessToken) {
            throw new Error("no access token — adapter not ready");
        }
        ...
        if (cached) {
            ...
            if (getResp.status === 443) {
                this.log.warn(
                    `motion write for ${camId.slice(0, 8)} skipped — camera in privacy mode (HTTP 443)`,
                );
                return false; // skip ack — keep last known value
            }
            motion = (getResp.data as Record<string, unknown>) ?? {};
        }
        ...
        const resp = await this._httpClient.put<unknown>(url, motion, {
            headers,
            validateStatus: (s) => (s >= 200 && s < 300) || s === 443,
        });
        if (resp.status === 443) {
            this.log.warn(
                `motion write for ${camId.slice(0, 8)} rejected — camera in privacy mode (HTTP 443)`,
            );
            return false;
        }
        this._motionCache.set(camId, { ...motion });
        this.log.info(
            `Motion config ${JSON.stringify(delta)} for ${camId.slice(0, 8)} (HTTP ${resp.status})`,
        );
        return true;
    }
    ...
    private async _handleIntrusionWrite(
        camId: string,
        delta: { sensitivity?: number; distance?: number; detectionMode?: string },
    ): Promise<void> {
        if (!this._currentAccessToken) {
            throw new Error("no access token — adapter not ready");
        }
     ...
     */
    private async _handleRecordSoundWrite(camId: string, on: boolean): Promise<boolean> {
        if (!this._currentAccessToken) {
            throw new Error("no access token — adapter not ready");
        }
        const url = `${CLOUD_API}/v11/video_inputs/${camId}/recording_options`;
        ...
        if (resp.status === 443) {
            this.log.warn(
                `record_sound write for ${camId.slice(0, 8)} rejected — camera in privacy mode (HTTP 443)`,
            );
            return false;
        }
        this.log.info(
     ...
     */
    private async _withCameraLock<T>(
     ...
     */
    private async _handleAudioDetectionWrite(
        camId: string,
        field: "detectGlassBreak" | "detectFireAlarm",
        on: boolean,
    ): Promise<boolean> {
        if (!this._currentAccessToken) {
            throw new Error("no access token — adapter not ready");
        ...
        return this._withCameraLock(this._audioDetectionLocks, camId, async () => {
            ...
            if (cached) {
                ...
                if (getResp.status === 443) {
                    this.log.warn(
                        `${field} write for ${camId.slice(0, 8)} skipped — privacy mode (443)`,
                    );
                    return false;
                }
                cfg = (getResp.data as Record<string, unknown>) ?? {};
            ...
            if (resp.status === 443) {
                this.log.warn(
                    `${field} write for ${camId.slice(0, 8)} rejected — privacy mode (443)`,
                );
                return false;
            }
     ...
     */
    private async _handleLensElevationWrite(camId: string, elevation: number): Promise<void> {
        if (!this._currentAccessToken) {
            throw new Error("no access token — adapter not ready");
        }
        ...
        this.log.info(
            `Lens elevation set to ${clamped} m for ${camId.slice(0, 8)} (HTTP ${resp.status})`,
        );
    }
     ...
     */
    private async _handleDarknessThresholdWrite(camId: string, pct: number): Promise<void> {
        if (!this._currentAccessToken) {
            throw new Error("no access token — adapter not ready");
        }
        ...
        if (!cached) {
            try {
                ...
                cached = (getResp.data as Record<string, unknown>) ?? {};
                ...
                const msg = err instanceof Error ? err.message : String(err);
                this.log.warn(
                    `darkness_threshold: aborting write for ${camId.slice(0, 8)} — could not ` +
                        `load current lighting config (${msg}), refusing to guess softLightFading`,
                );
                throw new Error(
                    `darkness_threshold: could not load current lighting config for ${camId.slice(0, 8)}`,
                );
            }
        }
        const softLightFading =
            typeof cached.softLightFading === "boolean" ? cached.softLightFading : true;
        ...
        this.log.info(
            `Darkness threshold set to ${clamped}% (${boschValue}) for ${camId.slice(0, 8)} (HTTP ${resp.status})`,
        );
    }
    ...
    private async _handleAlarmSettingsWrite(
        ...
        if (delta.alarmActivationDelaySeconds !== undefined) {
            cfg.alarmActivationDelaySeconds = Math.max(
                0,
                Math.min(600, delta.alarmActivationDelaySeconds),
            );
        }
     ...
     */
    private async _handlePrivacySoundWrite(camId: string, enabled: boolean): Promise<void> {
        ...
        if (this._isFeatureUnsupported(camId, "privacy_sound")) {
            this.log.debug(
                `privacy_sound_override skipped for ${camId.slice(0, 8)} — cached as unsupported (442)`,
            );
            return;
        }
        if (!this._currentAccessToken) {
        ...
        const url = `${CLOUD_API}/v11/video_inputs/${camId}/privacy_sound_override`;
        const resp = await this._httpClient.put<unknown>(
            url,
            { result: enabled },
            {
        ...
        if (resp.status === 442) {
            ...
            this.log.warn(
                `privacy_sound_override not supported on camera ${camId.slice(0, 8)} (HTTP 442) — ` +
                    `caching as unsupported; further writes will short-circuit`,
            );
            return;
        }
        this.log.info(
            `Privacy sound ${enabled ? "enabled" : "disabled"} for ${camId.slice(0, 8)} (HTTP ${resp.status})`,
        );
    }
     ...
     * @param camId  Camera UUID
     */
    private async _pollPrivacySound(token: string, camId: string): Promise<void> {
        ...
        try {
            ...
            this._recordPollResult(camId, "privacy_sound", false);
            this.log.debug(
                `Privacy sound poll for ${camId.slice(0, 8)} failed: ` +
                    `${err instanceof Error ? err.message : String(err)}`,
            );
        }
    }
     ...
     */
    private async _handleAutofollowWrite(camId: string, enabled: boolean): Promise<void> {
        const cam = this._cameras.get(camId);
        if (!cam || cam.panLimit <= 0) {
            throw new Error(
                `Autofollow not supported for camera ${camId.slice(0, 8)} (panLimit=0)`,
            );
        }
        if (!this._currentAccessToken) {
            throw new Error("no access token — adapter not ready");
        }
        const url = `${CLOUD_API}/v11/video_inputs/${camId}/autofollow`;
        const resp = await this._httpClient.put<unknown>(
            url,
            { result: enabled },
            {
        ...
        this.log.info(
            `Autofollow ${enabled ? "enabled" : "disabled"} for ${camId.slice(0, 8)} (HTTP ${resp.status})`,
        );
    }
     ...
     */
    private async _pollAutofollow(token: string, camId: string): Promise<void> {
        // v0.9.1: skip if cached unsupported or in backoff window.
        ...
        try {
            ...
            this._recordPollResult(camId, "autofollow", false);
            this.log.debug(
                `Autofollow poll for ${camId.slice(0, 8)} failed: ` +
                    `${err instanceof Error ? err.message : String(err)}`,
            );
        }
    }
     ...
     * @param camId  Camera UUID
     */
    private async _handleMarkAllRead(camId: string): Promise<void> {
        if (!this._currentAccessToken) {
            throw new Error("no access token — adapter not ready");
        }
        const token = this._currentAccessToken;
        ...
        const listUrl = `${CLOUD_API}/v11/events?videoInputId=${camId}&limit=20`;
        const listResp = await this._httpClient.get<unknown>(listUrl, {
            headers: { Authorization: `Bearer ${token}`, Accept: "application/json" },
            validateStatus: (s) => s >= 200 && s < 300,
        });
        const events = Array.isArray(listResp.data)
            ? (listResp.data as Array<Record<string, unknown>>)
            : [];
        if (events.length === 0) {
        ...
        for (const ev of events) {
            ...
            try {
                await this._httpClient.put<unknown>(
                    `${CLOUD_API}/v11/events`,
                    { id: evId, isRead: true },
        ...
        this.log.info(
            `mark_all_read: marked ${marked}/${events.length} events as read for ${camId.slice(0, 8)}`,
        );
    }
    ...
    private async _handlePanWrite(camId: string, position: number): Promise<number | null> {
        ...
        const url = `${CLOUD_API}/v11/video_inputs/${camId}/pan`;
        const resp = await this._httpClient.put<unknown>(
            url,
            {
     ...
     * @param brightness 0..100
     */
    private async handleLedGroupBrightnessUpdate(
        camId: string,
        groupKey: "topLedLightSettings" | "bottomLedLightSettings",
        brightness: number,
    ): Promise<void> {
        const cam = this._cameras.get(camId);
        if (!cam || cam.generation < 2 || cam.featureLight !== true) {
            this.log.warn(
                `${groupKey === "topLedLightSettings" ? "top_led_brightness" : "bottom_led_brightness"} ` +
        ...
        if (!current) {
            current =
                (await fetchLightingState(this._httpClient, this._currentAccessToken, camId)) ??
                undefined;
            if (!current) {
                const dp =
                    groupKey === "topLedLightSettings"
                        ? "top_led_brightness"
                        : "bottom_led_brightness";
                this.log.warn(
                    `${dp}: aborting write for ${camId.slice(0, 8)} — no cached lighting ` +
        ...
        const safeVal = Number.isFinite(brightness) ? brightness : 0;
        const next = buildLedGroupBrightnessUpdate(current, groupKey, safeVal);
        const result = await putLightingState(
            this._httpClient,
            this._currentAccessToken,
            camId,
        ...
        this.log.info(
            `${dp} for camera ${camId.slice(0, 8)}: brightness=${result[groupKey].brightness}`,
        );
    }
     ...
     *   light.py polarity.
     */
    private async handleFrontLightWhiteBalanceUpdate(
        camId: string,
        whiteBalance: number,
    ): Promise<void> {
        const cam = this._cameras.get(camId);
        if (!cam || cam.generation < 2 || cam.featureLight !== true) {
            this.log.warn(
                `front_light_white_balance write for ${camId.slice(0, 8)} ignored — ` +
        ...
        if (!current) {
            ...
            if (!current) {
                this.log.warn(
                    `front_light_white_balance: aborting write for ${camId.slice(0, 8)} — no ` +
        ...
        const next = buildFrontLightWhiteBalanceUpdate(current, safeVal);
        const result = await putLightingState(
            this._httpClient,
            this._currentAccessToken,
            camId,
        ...
        await this.upsertState(
            ...
            result.frontLightSettings.whiteBalance ?? -1,
        );
        this.log.info(
            `front_light_white_balance for camera ${camId.slice(0, 8)}: ` +
     ...
     */
    private async _handleSoftLightFadingWrite(camId: string, enabled: boolean): Promise<boolean> {
        const cam = this._cameras.get(camId);
        if (
            !cam ||
            cam.generation < 2 ||
            ...
            this.log.warn(
                `soft_light_fading write for ${camId.slice(0, 8)} ignored — Gen2 Outdoor only`,
            );
            return false;
        }
        if (!this._currentAccessToken) {
        ...
        if (!cached) {
            try {
                ...
                const msg = err instanceof Error ? err.message : String(err);
                this.log.warn(
                    `soft_light_fading: aborting write for ${camId.slice(0, 8)} — could not ` +
        ...
        const darknessThreshold =
            typeof cached.darknessThreshold === "number" ? cached.darknessThreshold : 0.5;
        ...
        this._globalLightingCache.set(camId, { ...cached, ...body });
        this.log.info(
            `soft_light_fading set to ${enabled} for ${camId.slice(0, 8)} (HTTP ${resp.status})`,
        );
        return true;
    }
     ...
     */
    private async _handleRenameWrite(camId: string, newTitle: string): Promise<boolean> {
        if (!this._currentAccessToken) {
            throw new Error("no access token — adapter not ready");
        }
        const cam = this._cameras.get(camId);
        ...
        try {
            const resp = await this._httpClient.put<unknown>(
                ...
                { videoInputId: camId, title: newTitle, timeZone },
                {
                    headers: {
                        Authorization: `Bearer ${this._currentAccessToken}`,
                        "Content-Type": "application/json",
     ...
     * @param camId Camera UUID
     */
    private async _handleSoftResetWrite(camId: string): Promise<void> {
        ...
        const safeCamId = sanitizeId(camId);
        try {
            const resp = await this._httpClient.put<unknown>(
                `${CLOUD_API}/v11/video_inputs/${safeCamId}/soft_reset`,
                null,
                {
     ...
     * @param camId Camera UUID
     */
    private async _handleHardResetWrite(camId: string): Promise<void> {
        try {
            const cam = this._cameras.get(camId);
            ...
            if (!cam || !confirmVal || confirmVal !== cam.name || !withinWindow) {
                const expiredNote =
                    confirmVal && confirmVal === cam?.name && !withinWindow
                        ? " (60s confirm window expired — re-type it)"
                        : "";
                this.log.warn(
                    `hard_reset for ${camId.slice(0, 8)} REJECTED — hard_reset_confirm must hold ` +
            ...
            const safeCamId = sanitizeId(camId);
            const resp = await this._httpClient.put<unknown>(
                `${CLOUD_API}/v11/video_inputs/${safeCamId}/hard_reset`,
                null,
                {
                    headers: { Authorization: `Bearer ${this._currentAccessToken}` },
                    validateStatus: () => true,
            ...
            if (resp.status >= 200 && resp.status < 300) {
                this.log.warn(
                    `DESTRUCTIVE hard reset (factory reset) triggered for camera ${camId.slice(0, 8)} — ` +
     ...
     * @param camId Camera UUID
     */
    private async _handleAiAnalyzeTrigger(camId: string): Promise<void> {
    ...
    private async _doAiAnalyzeTrigger(camId: string): Promise<void> {
        // Bug-hunt finding: unlike _handleHardResetWrite, this never checked
        // the camera actually exists before proceeding.
        if (!this._cameras.has(camId)) {
        ...
        const cam = this._cameras.get(camId);
        const endpointUrl =
            typeof this.config.ai_analysis_endpoint_url === "string"
                ? this.config.ai_analysis_endpoint_url.trim()
                : "";
        if (this.config.ai_analysis_enabled !== true || !endpointUrl) {
            this.log.warn(
                `ai_analyze for ${camId.slice(0, 8)} ignored — AI Analysis is disabled or has ` +
        ...
        const ssrfCheck = await validateAiEndpointUrl(endpointUrl);
        if (!ssrfCheck.ok) {
            this.log.error(
                `ai_analyze for ${camId.slice(0, 8)} REJECTED — configured endpoint URL failed ` +
        ...
        const apiKey =
            typeof this.config.ai_analysis_api_key === "string"
        ...
        try {
            ...
            if (resp.status < 200 || resp.status >= 300) {
                this.log.warn(
                    `ai_analyze for ${camId.slice(0, 8)}: endpoint returned HTTP ${resp.status}`,
                );
                return;
            }
            const data = resp.data as Record<string, unknown>;
            ...
            this.log.info(
                `AI analysis for camera ${camId.slice(0, 8)}: score=${score} description="${description.slice(0, 60)}"`,
            );
        } catch (err: unknown) {
            const msg = err instanceof Error ? err.message : String(err);
    ...
    private static _countDailyEvents(
        ...
        nowMs?: number,
    ): { today: number; movement: number; audio: number } {
        const todayLocal = BoschSmartHomeCamera._localDateKey(new Date(nowMs ?? Date.now()));
        let today = 0;
        let movement = 0;
        let audio = 0;
        for (const ev of events) {
            const raw = (ev.timestamp ?? ev.createdAt ?? "") as string;
            const evDate = new Date(BoschSmartHomeCamera.normaliseBoschTimestamp(raw));
            if (Number.isNaN(evDate.getTime())) {
                continue;
            }
            if (BoschSmartHomeCamera._localDateKey(evDate) !== todayLocal) {
                continue;
            }
            today++;
            const type = ((ev.eventType ?? "") as string).toUpperCase();
            if (type === "MOVEMENT") {
                movement++;
    ...
    private static _localDateKey(d: Date): string {
    ...
    private async fetchAndProcessEvents(): Promise<void> {
        ...
        for (const [camId] of this._cameras) {
            try {
                ...
                const url = `${CLOUD_API}/v11/events?videoInputId=${camId}&limit=20`;
    ...
    private async handlePrivacyToggle(camId: string, enabled: boolean): Promise<void> {
        ...
        if (this._currentAccessToken) {
            const url = `${CLOUD_API}/v11/video_inputs/${camId}/privacy`;
    ...
    private async _applyLightingState(
        ...
        if (isGen2) {
            const base = `${CLOUD_API}/v11/video_inputs/${camId}/lighting/switch`;
            try {
            ...
            const url = `${CLOUD_API}/v11/video_inputs/${camId}/lighting_override`;
            const body = {
     ...
     */
    private async _publishMotionEventImage(camId: string, buf: Buffer): Promise<void> {
    ...
    private async _doSnapshotTrigger(
        camId: string,
        opts: { asMotionEvent?: boolean } = {},
    ): Promise<void> {
        ...
        try {
            ...
            const privacyOn =
                (await this.getStateAsync(`cameras.${camId}.privacy_enabled`))?.val === true;
            const useMjpeg =
                this.config.use_mjpeg_snapshot !== false && // default true
                cam?.generation === 2 &&
                lanReachable &&
                !privacyOn &&
                session.digestUser &&
            ...
            if (useMjpeg) {
                ...
                const mjpegBuf = await fetchMjpegSnapshot(
                    lanIp,
                    443,
                    session.digestUser,
                    session.digestPassword,
                ...
                if (mjpegBuf !== null) {
                    ...
                    if (fails === BoschSmartHomeCamera.MJPEG_FASTPATH_MAX_FAILS) {
                        this.log.debug(
                            `MJPEG fast-path disabled for ${camId.slice(0, 8)} this session ` +
                                `after ${fails} failures — using snap.jpg`,
                        );
                    } else {
                        this.log.debug(
                            `MJPEG snapshot failed for ${camId.slice(0, 8)}, falling back to snap.jpg`,
                        );
                    }
     ...
     */
    private async _fetchSnapJpgWithRetry(
        camId: string,
        snapUrl: string,
     ...
     */
    private get _pollIntervalMs(): number {
    ...
    private async _handleSessionLimitError(camId: string): Promise<void> {
        ...
        try {
            ...
            this.log.debug(
                `session_limit_hit DP write failed for ${camPrefix} (non-fatal): ` +
                    `${err instanceof Error ? err.message : String(err)}`,
            );
        }
        ...
        this.setTimeout(() => {
            ...
            void this.ensureLiveSession(camId)
                .then(async () => {
                    ...
                    await this.setStateAsync(`cameras.${camId}.session_limit_hit`, false, true);
                    this.log.info(
                        `[session-quota] Session-quota recovered for camera ${camPrefix}`,
                    );
                })
                .catch((retryErr: unknown) => {
                    const msg = retryErr instanceof Error ? retryErr.message : String(retryErr);
                    this.log.debug(
                        `[session-quota] Auto-retry still failed for ${camPrefix}: ${msg}`,
                    );
                    ...
                    if (retryErr instanceof SessionLimitError) {
    ...
    private async markCameraReachability(camId: string, reachable: boolean): Promise<void> {
        ...
        if (failures >= BoschSmartHomeCamera.OFFLINE_THRESHOLD) {
            ...
            if (this._isCameraMaintenanceActive() && this._liveSessions.has(camId)) {
                this.log.debug(
                    `[status] suppressing offline flip for ${camId.slice(0, 8)} — ` +
     ...
     * locally streaming.
     */
    private _isCameraMaintenanceActive(): boolean {
        const mw = this._lastMaintenanceWindow;
        return mw !== null && mw.camera_relevant === true && classifyState(mw) === "active";
    }
     ...
     */
    private async _maybeannounceCameraStatus(camId: string, newStatus: string): Promise<void> {
